...
| `profile` | welcome → create profile | Continue | grown-up setup cue, profile nickname field, and on-device/transient-media privacy copy are visible | `$TMPDIR/brushtales-prototype/profile-430.png`; no avatar selection |
| `opening` | welcome → start a new story | Begin story | Act 1 audiobook heading, one clearly labeled start action, and safe-brushing caregiver reminder are exposed | Opening audiobook sets the scene before the brushing activity; `preflight` remains a direct-query alias |
